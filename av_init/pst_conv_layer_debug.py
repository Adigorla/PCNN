...
def make_nn_funs(input_shape, layer_specs, L2_reg):
    ...
    def predictions(W_vect, inputs):
        ...
        for layer in layer_specs:
            ...
            #if check == 0:
            #    print("weights from prediction function call")
            #    print(cur_weights)
            #    check += 1
        return cur_units
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for idxs in batch_idxs:
            ...
            # debugging statements to compare gradient value for each parameter with value obtained from finite difference method after each batch - this can be modified to every epoch 
            index = 0
            W_t0 = np.copy(W)
            W_t0[index]=W_t0[index]+.000001

            W_t1 = np.copy(W)
            W_t1[index] = W_t1[index] - .000001


            lssfn0 = loss_fun(W_t0, train_images[idxs], train_labels[idxs])
            lssfn1 = loss_fun(W_t1, train_images[idxs], train_labels[idxs])
            print("parameter 1:")
            print("original W value: "+str(W[index]))
            print("loss 1: "+str(lssfn0))
            print("loss 2: " + str(lssfn1))
            finite_val = (lssfn0 - lssfn1)/.000002
            print("autograd: "+str(grad_W[index])+" finite grad: "+str(finite_val))


            W_t0 = np.copy(W)
            W_t0[1]=W_t0[1]+.000001

            W_t1 = np.copy(W)
            W_t1[1] = W_t1[1] - .000001
            ...
            finite_val = (lssfn0 - lssfn1)/.000002
            print("parameter 2:")
            print("original W value: "+str(W[1]))
            print("autograd: "+str(grad_W[1])+" finite grad: "+str(finite_val))

            W_t0 = np.copy(W)
            W_t0[2] = W_t0[2] + .000001

            W_t1 = np.copy(W)
            W_t1[2] = W_t1[2] - .000001

            lssfn0 = loss_fun(W_t0, train_images[idxs], train_labels[idxs])
            lssfn1 = loss_fun(W_t1, train_images[idxs], train_labels[idxs])
            finite_val = (lssfn0 - lssfn1) / .000002
            print("parameter 3:")
            print("original W value: "+str(W[2]))
            print("autograd: " + str(grad_W[2]) + " finite grad: " + str(finite_val))

	    #value obtained from dummy variable to verify that the gradient method is functioning properly
 
            W_t0 = np.copy(W)
            W_t0[3] = W_t0[3] + .000001

            W_t1 = np.copy(W)
            W_t1[3] = W_t1[3] - .000001

            lssfn0 = loss_fun(W_t0, train_images[idxs], train_labels[idxs])
            lssfn1 = loss_fun(W_t1, train_images[idxs], train_labels[idxs])
            finite_val = (lssfn0 - lssfn1) / .000002
            print("parameter 4:")
            print("original W value: "+str(W[3]))
            print("autograd: " + str(grad_W[3]) + " finite grad: " + str(finite_val))
